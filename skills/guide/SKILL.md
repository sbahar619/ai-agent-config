...
name: guide
description: >-
  Produce a numbered step-by-step guide with prerequisites and per-step
  verification. Use when the task requires a how-to or walkthrough document.
...
A task, tool, process, or question (e.g. "set up mTLS between two services",
"rotate a Kubernetes secret").

## Rules

- Number steps sequentially; one action per step
- Order steps so each depends only on steps already completed — no forward
  references, no skipped prerequisites
...
- Where a step has a way to verify it worked, include it in that step
